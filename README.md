...
│   ├── components/         # UI 组件
│   │   ├── workflow/       #   工作流时间线 (思考链 / 工具调用 / 确认节点)
│   │   ├── preview/        #   文档预览 (Markdown / PDF Canvas)
│   │   ├── settings/       #   设置面板 (8 个标签页)
│   │   ├── sidebar/        #   右侧栏 (文件树 / Todo / Agent 信息)
│   │   └── layout/         #   布局 (顶栏 / 主区域 / 输入区 / 窗口控件)
│   ├── stores/             # Zustand 状态管理
│   ├── hooks/              # Agent 交互核心 Hook
│   └── services/           # Tauri 通信封装
│
├── src-tauri/              # Rust 后端
│   └── src/
│       ├── commands/       #   Tauri 命令层 (12 个模块, 40+ 命令)
│       ├── services/       #   业务逻辑层
│       │   ├── agent/      #     Agent 调度引擎 (Tool Calling 循环)
│       │   ├── llm/        #     LLM 多 Provider 适配 (OpenAI / Anthropic / Gemini)
│       │   ├── skill/      #     技能引擎 (注册表 + 6 个内置技能)
│       │   ├── tool/       #     工具引擎 (8 个文件系统工具, 始终启用)
│       │   └── document/   #     Python Sidecar 进程管理
│       ├── db/             #   SQLite 数据层 (会话 / 消息 / 快照 / 模板)
│       ├── config/         #   JSON 配置管理
│       ├── models/         #   数据模型
│       ├── events/         #   事件系统
│       └── errors.rs       #   统一错误码
