...
pub fn run() {
    ...
    builder
        .setup(|app| {
            ...
            // 初始化数据库（含损坏检测和自动恢复）
            let db_path = app_data_dir.join("samoyed_work.db");
            ...
            // 解析 Python 可执行文件路径
            // 优先级：
            // 1. 环境变量 SAMOYED_WORK_PYTHON（开发模式覆盖，最高优先级）
            ...
            let python_path = if let Ok(p) = std::env::var("SAMOYED_WORK_PYTHON") {
                log::info!("使用环境变量 SAMOYED_WORK_PYTHON 指定的 Python: {}", p);
                p
            } else {
