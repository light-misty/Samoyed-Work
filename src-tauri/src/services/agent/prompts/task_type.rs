...
impl TaskType {
    ...
    pub fn from_tool_name(tool_name: &str) -> Self {
        match tool_name {
            ...
            "list_directory" | "search_files" | "read_file" | "file_info" | "file_exists"
            | "delete_file" | "create_directory" | "write_text_file" => TaskType::FileSystem,
            _ => TaskType::Unknown,
        }
    }
...
mod tests {
    ...
    fn test_from_user_message_docx() {
        assert_eq!(
            TaskType::from_user_message("帮我创建一份项目周报"),
            TaskType::Docx
        );
        assert_eq!(TaskType::from_user_message("生成Word文档"), TaskType::Docx);
        assert_eq!(
            TaskType::from_user_message("写一个合同文档"),
            TaskType::Docx
        );
        assert_eq!(
            TaskType::from_user_message("修改报告.docx的内容"),
            TaskType::Docx
        );
        assert_eq!(
            TaskType::from_user_message("读取data.docx的内容"),
            TaskType::Docx
        );
    }
    ...
    fn test_from_user_message_pptx() {
        assert_eq!(
            TaskType::from_user_message("创建PPT演示文稿"),
            TaskType::Pptx
        );
    ...
    fn test_from_user_message_pdf() {
        ...
        assert_eq!(
            TaskType::from_user_message("把方案.docx转成PDF"),
            TaskType::Pdf
        );
    ...
    fn test_from_user_message_markdown() {
        assert_eq!(
            TaskType::from_user_message("创建markdown文件"),
            TaskType::Markdown
        );
        assert_eq!(
            TaskType::from_user_message("写一个.md文件"),
            TaskType::Markdown
        );
    }
    ...
    fn test_from_user_message_filesystem() {
        assert_eq!(
            TaskType::from_user_message("列出目录内容"),
            TaskType::FileSystem
        );
        assert_eq!(
            TaskType::from_user_message("搜索文件"),
            TaskType::FileSystem
        );
    }
    ...
    fn test_from_user_message_unknown() {
        ...
        assert_eq!(
            TaskType::from_user_message("什么是Samoyed Work"),
            TaskType::Unknown
        );
    }
    ...
    fn test_from_tool_name() {
        ...
        assert_eq!(
            TaskType::from_tool_name("list_directory"),
            TaskType::FileSystem
        );
