...
| 工作流节点 | 完成 | WorkflowTimeline + 7 种节点组件（UserNode、ThinkingNode、ReplyNode、ToolNode、ResultNode、ConfirmNode、WorkflowNode），支持展开/折叠 |
| 侧边栏区块 | 完成 | AgentInfoSection、FileTreeSection、TodoSection、TokenSection 四个区块 |
| 设置对话框 | 完成 | SettingsDialog + 5 个标签页（LLMConfig、GeneralTab、SkillsTab、TemplatesTab、WorkspaceTab），Provider 添加/编辑/删除/测试/设默认、工作区添加/切换/移除均已对接后端 |
| 预览面板 | 完成 | PreviewOverlay 支持文档预览和基础差异对比 |
| 会话历史 | 完成 | HistoryPanel 支持会话列表展示和切换 |
| 状态管理 | 完成 | 6 个 Zustand Store（useWorkflowStore、useSessionStore、useWorkspaceStore、useSettingsStore、useFileTreeStore、useTokenStore）全部实现，已连接后端 Tauri 命令 |
...
| 事件系统 | 完成 | AgentEmitter 封装全部 10 种事件类型（thinking, content, tool_call, tool_result, confirm, todo_update, done, error, stopped, session_updated），含发射方法和 payload 类型定义 |
| LLM 服务 | 完成 | OpenAI 兼容 API 适配器完整实现，支持流式(SSE)和非流式调用，指数退避重试，Default/Fallback 路由，list_providers 返回完整元数据 |
| Agent 执行器 | 完成 | 完整实现：包含确认通道（高风险 Skill 前发射 agent:confirm 并通过 oneshot channel 等待用户决策）、Todo 进度更新（agent:todo_update）、Tool Calling 循环，支持停止检查和最大迭代限制，增量持久化防止崩溃丢失消息 |
| Skill 注册表 | 完成 | Skill trait + 注册表框架 + 9 个内置 Skills（含参数 schema）+ 禁用/启用能力（运行时修改+配置持久化），线程安全（Mutex 保护），支持获取 Arc 引用避免持锁执行 |
...
| 处理器 | 状态 | 功能 |
|--------|------|------|
| Word 处理器 | 完成 | generate、read、modify、convert（支持 md/txt）、analyze 全部实现 |
| Excel 处理器 | 完成 | generate、read、modify、analyze 实现；convert 暂未实现 |
| PDF 处理器 | 完成 | generate（reportlab）、read（PyMuPDF/pdfminer）、analyze 实现；modify 和 convert 暂不支持 |
| PPT 处理器 | 完成 | generate、read、modify、analyze 实现；convert 暂未实现 |
| Markdown 处理器 | 完成 | generate、read、modify、analyze 实现；convert 暂未实现 |
| main.py | 完成 | stdin/stdout JSON 协议通信，日志输出到 log/sidecar.log，支持 UTF-8 BOM 处理 |
...
│   ├── workflow/         # WorkflowTimeline + 7 种节点组件（UserNode, ThinkingNode, ReplyNode, ToolNode, ResultNode, ConfirmNode, WorkflowNode）
│   ├── sidebar/          # AgentInfoSection, FileTreeSection, TodoSection, TokenSection
│   ├── preview/          # PreviewOverlay 预览面板
│   ├── settings/         # SettingsDialog + 5 个标签页（LLMConfig, GeneralTab, SkillsTab, TemplatesTab, WorkspaceTab）
│   ├── session/          # HistoryPanel
│   └── common/           # Button, Icon
├── stores/               # 6 个 Zustand Store（useWorkflowStore, useSessionStore, useWorkspaceStore, useSettingsStore, useFileTreeStore, useTokenStore）
...
│   │   └── document.rs   # 文档操作
│   ├── services/         # 服务层
│   │   ├── agent/        # Agent 执行器（executor.rs 含确认通道+Todo 事件+增量持久化、context.rs）
│   │   ├── llm/          # LLM 服务（router.rs 完整 Provider 元数据，openai_adapter.rs 流式 SSE）
│   │   ├── skill/        # Skill 系统（registry.rs, builtin.rs 含 9 个内置技能）
...
**增量持久化机制**：
- executor 每轮迭代后调用 `persist_new_messages` 将新增消息写入数据库
- 防止崩溃或用户中断时丢失对话历史
- 通过 `AgentContext` 追踪未持久化消息
...
| `delete_document` | 删除文档 | Rust 原生（支持备份+安全校验） |
...
**Skill 禁用机制**：
- 通过 `toggle_skill` 命令启用/禁用 Skill
- 禁用状态持久化到 `app_settings.json` 的 `disabled_skills` 字段
- 应用启动时从配置加载已禁用 Skill 列表

**Skill 安全机制**：
- `delete_document`、`search_documents`、`list_workspace` 等 Skill 接收 `workspace_root` 参数
- 执行前校验目标路径是否在工作区内，防止路径遍历攻击
- `workspace_root` 由 executor 从应用配置注入，不依赖 LLM 提供的值
...
- **并发安全**：Skill 注册表使用 Mutex 保护，执行时获取 Arc 引用后释放锁，避免在 Sidecar 通信等耗时操作期间阻塞注册表
