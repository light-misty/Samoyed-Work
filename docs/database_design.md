# Samoyed Work 数据库设计文档

> 版本：0.1.6
> 最后更新：2026-06-14
> 适用项目：Samoyed Work文档处理桌面应用
...
```
{app_data_dir}/samoyed_work.db
```

其中 `{app_data_dir}` 为 Tauri 的 `app.path().app_data_dir()` 返回路径（Windows 上为 `%APPDATA%/samoyed-work`）。
