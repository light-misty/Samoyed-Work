...
- 可配置环境变量 `SAMOYED_WORK_PYTHON` 指定 Python 解释器路径
...
- write_script 写入路径限定为系统临时目录 `<temp_dir>/samoyed_work/scripts/`
...
6. Python Sidecar 进程启动（路径检测：环境变量 `SAMOYED_WORK_PYTHON` / py / python / python3）
