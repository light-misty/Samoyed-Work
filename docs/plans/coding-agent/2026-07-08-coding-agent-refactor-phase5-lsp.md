...
3. **LSP 工具**:OpenCode 暴露单一 `lsp` 工具给 Agent(实验性,需环境变量 `OPENCODE_EXPERIMENTAL_LSP_TOOL` 开启),通过 `operation` 参数路由不同操作:
   ...
   - 注:OpenCode 原生支持 9 种操作,其中 `incomingCalls`/`outgoingCalls` 在 Samoyed Work 中合并为 `call_hierarchy` 操作(通过子参数 `direction` 区分方向)
...
**验证**:
- `cargo build -p samoyed_work_lib` 成功
...
use samoyed_work_lib::services::lsp::{LanguageRouter, LspResultCache};
use samoyed_work_lib::models::lsp::*;
use std::path::PathBuf;
...
use samoyed_work_lib::services::lsp::*;
use samoyed_work_lib::services::tool::trait_def::Tool;
use samoyed_work_lib::services::tool::builtin::lsp_tools::*;
use serde_json::json;
use std::path::PathBuf;
...
本阶段实现了完整的 LSP 集成,让 Samoyed Work 获得了现代 IDE 级别的代码理解能力:
...
至此,Samoyed Work 编程 Agent 改造的 5 个阶段全部完成:
...
Samoyed Work 已从文档处理 Agent 成功改造为编程 Agent,具备:
- 完整的代码生成与执行能力(阶段 1)
