# Samoyed Work 编程 Agent 改造 - 阶段 2:权限系统与 Agent 模式
...
在阶段 1 已经建立核心编程能力(文件读写、编辑、搜索、命令执行)的基础上,本阶段将 Samoyed Work 升级为具备精细化权限管控和模式切换的编程 Agent:
...
impl PermissionEvaluator {
    ...
    pub fn evaluate(request: &PermissionRequest, rules: &[PermissionRule]) -> PermissionDecision {
        ...
        if matching_rules.is_empty() {
            ...
            // Samoyed Work 选择默认 allow 以减少对用户操作的干扰,可通过配置调整为 ask)
...
```rust
use samoyed_work_lib::services::permission::*;
use samoyed_work_lib::models::permission::{PermissionRule, PermissionRuleFilter};

#[tokio::test]
async fn test_full_permission_flow() {
    // 1. 初始化权限系统
    let tmp = tempfile::NamedTempFile::new().unwrap();
    let db = std::sync::Arc::new(samoyed_work_lib::db::Database::new(std::path::Path::new(tmp.path())).unwrap());
...
async fn test_plan_mode_blocks_modification() {
    use samoyed_work_lib::services::agent::AgentMode;
...
async fn test_agent_mode_manager() {
    use samoyed_work_lib::services::agent::AgentModeManager;

    let manager = AgentModeManager::new();

    // 默认 Build 模式
    assert_eq!(manager.get_mode("s1").await, samoyed_work_lib::services::agent::AgentMode::Build);
...
检查 `log/samoyed_work.log` 中包含以下关键日志:
- `已插入权限规则`
- `已更新权限规则`
- `检测到 Doom loop`
- `会话 X 模式切换`
