...
Samoyed Work 起初定位为 AI 文档处理桌面应用,基于 Tauri 2.x (Rust + React/TypeScript) 构建,通过 4 个文档 Handler (docx/xlsx/pptx/pdf,经 Python Sidecar 执行)完成文档的生成、读取、修改、格式转换。
...
> **重要说明**:`Document` 模式为 Samoyed Work 原创设计,OpenCode 实际只有 `build`/`plan` 两个 primary 模式(外加 `general`/`explore` 等 subagent)。Samoyed Work 新增 Document 模式的目的是保留原有的文档处理能力,使应用在获得编程 Agent 能力的同时不丢失文档处理特色。
...
- **保留良好基础**:复用 Samoyed Work 现有的 LlmRouter、流式事件机制、确认通道、增量持久化、Scratchpad 等已验证的设计
- **分阶段交付**:按依赖关系分 5 个阶段,每阶段独立可交付、可测试
- **质量优先**:每阶段配套测试用例,遵循 TDD(测试驱动开发)

---

## 二、OpenCode vs Samoyed Work 架构对比

### 2.1 整体架构对比

| 维度 | OpenCode | Samoyed Work 现状 | 改造方向 |
|------|----------|---------------|----------|
...
```
System Prompt
├── 基础 prompt (系统内置核心提示词,OpenCode 原实现按 Provider 分类为 default.txt / anthropic / gpt / gemini 等,Samoyed Work 改造为统一基础 prompt,不按 Provider 区分)
...
**Samoyed Work 现有系统提示词(分层架构)**:

```
System Prompt
├── Layer 0: 身份层 (Samoyed Work 文档处理专家)
├── Layer 1: 规则层
...
| 工具类别 | OpenCode | Samoyed Work 现状 | 改造方向 |
|---------|----------|---------------|----------|
...
1. 实现三态权限系统(allow/deny/ask):
   - 权限类型:edit, read(Samoyed Work 扩展,OpenCode 原生不管控 read 权限), bash, webfetch, task, external_directory, doom_loop, document 等
    - 用户回复:once / reject
...
### 4.1 保留的 Samoyed Work 设计

以下 Samoyed Work 现有设计经过验证,将在改造中保留:
...
6. **缓存优化**:工具定义按字母序排序、工具结果截断、reasoning_content 压缩
7. **截断重试**:max_tokens 不足时翻倍重试
8. **网络监控**:断网暂停 + 恢复重试
9. **Tauri 无边框窗口**:保持现有 UI 框架
...
| **edit 工具误操作覆盖文件** | 阶段 1 数据丢失 | FileTime 锁断言;oldString 必须唯一匹配 |
...
> **Samoyed Work 扩展工具**(不在 OpenCode 13 个核心工具内,Samoyed Work 特色保留):
> - `write_script`:将智能体生成的脚本写入系统临时目录(Samoyed Work 特色)
> - `task`:子 Agent 委托工具(阶段4 实现)
> - `sourcecode`:代码语义搜索(阶段3 实现,基于 tree-sitter)
> - 文档 Handler(`docx`/`xlsx`/`pptx`/`pdf`):仅 Document 模式下动态启用

> **命名规则说明**：
> - Samoyed Work 原有工具：复合词保留下划线（如 `file_info`/`read_lines`/`remove_dir`/`write_script`），单词无下划线（如 `list`/`read`/`bash`）
