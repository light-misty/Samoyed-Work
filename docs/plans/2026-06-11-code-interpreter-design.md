...
```
用户请求 → LLM → 调用 Skill (docx_skill/xlsx_skill/pptx_skill/pdf_skill)
  → Rust 后端构造 JSON 请求 → stdin 发送给 Python Sidecar
  → Sidecar Handler 解析参数 → 调用 python-docx/openpyxl/python-pptx 等库
  → 返回 JSON 响应 → 文件生成到磁盘
```

每个 Skill 支持 5 种 action：`generate`/`read`/`modify`/`convert`/`analyze`。
...
│                      Agent (LLM)                             │
│                                                              │
│  读取/转换/分析 → docx_skill / xlsx_skill / pptx_skill / ...│
│                   (精简 Sidecar，快速可靠)                   │
│                   action: read / convert / analyze           │
│                                                              │
│  生成/修改     → code_interpreter_skill                      │
...
| 操作 | 执行方式 | 理由 |
|------|---------|------|
| **generate**（生成文档） | `code_interpreter_skill` | 需要灵活性：图表、数据处理、自定义排版 |
| **modify**（修改文档） | `code_interpreter_skill` | 需要灵活性：精细调整、复杂修改 |
| **read**（读取文档） | `docx_skill`/`xlsx_skill`/... | 固定 API 足够，快速可靠，低 token 消耗 |
| **convert**（格式转换） | `docx_skill`/`xlsx_skill`/... | 固定 API 足够，转换逻辑标准 |
| **analyze**（文档分析） | `docx_skill`/`xlsx_skill`/... | 固定 API 足够，统计信息标准 |
...
4. **用户可控**：Code Interpreter 可在设置中启用/禁用（Skill 禁用机制），始终需用户确认
...
#### 3.1.2 Skill action 精简

Rust 端 `builtin.rs` 中各 Skill 的 `execute()` 方法移除 `generate` 和 `modify` 分支。

注意：当前代码中各 Skill 的 `execute()` 方法使用共享函数（如 `execute_generate(&self.doc_service, "docx", params).await`），而非实例方法（如 `self.execute_generate(params).await`）。精简时需删除对应的 `match` 分支和共享函数。
...
#### 3.1.3 Skill parameters 精简

各 Skill 的 `parameters()` 中 `action` 的描述从 5 种缩减为 3 种：
...
| L6 | 用户确认 | `AgentExecutor.needs_confirmation()` | `code_interpreter_skill` 加入 `HIGH_RISK_SKILLS` 常量，始终需确认 |
...
### 3.5 Rust 端：新增 `code_interpreter_skill` Skill

在 `src-tauri/src/services/skill/builtin.rs` 中注册新的 Skill。命名遵循现有规范（`docx_skill`/`xlsx_skill`/`pptx_skill`/`pdf_skill`），使用 `code_interpreter_skill`。
...
注册时修改 `register_builtin_skills()` 函数：
...
这样 `needs_confirmation()` 的 `EditOnly` 分支会自动将 `code_interpreter_skill` 视为高风险，无需修改 `needs_confirmation()` 方法本身。

注意：精简后文档 Skill 不再有 `modify` action，`needs_confirmation()` 中 `matches!(name, "docx_skill" | "xlsx_skill" | "pptx_skill" | "pdf_skill") && params["action"].as_str() == Some("modify")` 这段检查将永远不会触发，可以在 Phase 2 中清理移除。

#### 3.6.2 注入 workspace_root

在 executor 的 `needs_workspace_root` 匹配列表中添加 `code_interpreter_skill`：
...
let needs_workspace_root = matches!(
    ...
    | "code_interpreter_skill"  // 新增：需要 workspace_root 作为 working_dir
);
```

#### 3.6.3 确认描述增强

在 `request_confirmation()` 的 `description` 匹配中添加 `code_interpreter_skill`：
...
`code_interpreter_skill` 的生成和修改操作都会修改文件，需要在操作前创建版本快照。修改 `extract_snapshot_paths()` 方法：
...
在 `request_confirmation()` 的 `risk_level` 匹配中添加 `code_interpreter_skill`：
...
集成方式：修改 `prompt_loader.rs` 的 `default_tool_strategy()` 方法，在现有工具策略末尾追加 `CODE_INTERPRETER_GUIDE` 内容。同时需要将 `default_tool_strategy()` 中"写入操作"部分的 generate/modify 指引改为指向 `code_interpreter_skill`：
...
当前 `SkillsTab.tsx` 中所有内置 Skill 都显示"始终启用"（`tool-always-on`），没有启用/禁用开关。需要新增开关功能。

当前 `SkillInfo` 类型（与 Rust 端 `models/skill.rs` 对齐，使用 camelCase）包含 `id`/`name`/`description`/`category`/`isBuiltin`/`enabled`/`version`/`paramsSchema`/`supportedTypes` 字段，其中 `enabled` 字段已存在但前端未使用。

修改方案：

```typescript
// SkillsTab.tsx 修改
// 1. 将内置 Skill 的"始终启用"改为可切换的 Switch 开关
// 2. code_interpreter_skill 标注为"高级功能"，启用时显示安全提示
// 3. 调用 toggle_skill 命令切换启用/禁用状态
...
<div className="skill-item-info">
  <div className="skill-name-row">
    ...
    {isCodeInterpreter && (
      <span className="skill-advanced-badge">高级</span>
    )}
  </div>
...
<div className="skill-toggle">
  <Switch
    checked={s.enabled}
    onCheckedChange={(checked) => handleToggleSkill(s.id, checked)}
  />
</div>
```

注意：需要确认 `toggle_skill` Tauri 命令是否已存在。如果不存在，需要在 Rust 端 `commands/skill.rs` 中新增该命令，调用 `SkillRegistry` 的禁用/启用方法，并持久化到 `AppSettings.disabled_skills`。
...
对于 `code_interpreter_skill`，Rust 端 `request_confirmation()` 的 `description` 已包含代码摘要（见 3.6.3），前端无需额外处理。`ConfirmNode` 组件本身无需修改，因为 `description` 字段已包含代码摘要信息。如果需要更丰富的代码展示（语法高亮、折叠），可在 `ConfirmNode` 中根据 `operationType === "code_interpreter_skill"` 条件渲染。
...
需要修改 `generateToolBrief()` 函数，添加 `code_interpreter_skill` 的分支：
...
Agent 的迭代循环（`AgentExecutor`）天然支持重试：LLM 收到错误后会尝试修改代码并重新调用 `code_interpreter_skill`。
...
| `src-tauri/src/services/skill/builtin.rs` | 新增 `CodeInterpreterSkill`；各 Skill 删除 `execute_generate()` 和 `execute_modify()`；`register_builtin_skills()` 注册新 Skill |
| `src-tauri/src/services/agent/executor.rs` | `HIGH_RISK_SKILLS` 新增 `code_interpreter_skill`；`needs_workspace_root` 新增 `code_interpreter_skill`；`request_confirmation()` description 新增 `code_interpreter_skill` 分支；`extract_snapshot_paths()` 新增 `code_interpreter_skill` 分支；`risk_level` 匹配新增 `code_interpreter_skill` |
...
| `src/components/settings/SkillsTab.tsx` | 新增 Skill 启用/禁用 Switch 开关，`code_interpreter_skill` 标注"高级" |
| `src/utils/format.ts` | `generateToolBrief()` 新增 `code_interpreter_skill` 分支 |
...
5. **修改 `builtin.rs`**：注册 CodeInterpreterSkill
6. **修改 `executor.rs`**：HIGH_RISK_SKILLS + workspace_root 注入 + 确认描述 + 快照路径 + 风险等级
7. **修改 `format.ts`**：`generateToolBrief()` 新增 `code_interpreter_skill` 分支
8. **端到端测试**：验证 Code Interpreter 完整链路

此阶段**不修改**现有 Skill 和 Sidecar Handler，确保现有功能不受影响。
...
3. **删除各 Skill 的 `execute_generate()` 和 `execute_modify()`**
...
| document_design.rs 精简后 LLM 仍尝试调用 generate/modify | 低 | Skill parameters 中 action enum 已移除，LLM 会收到错误并自动切换 |
...
| 测试项 | 说明 |
|--------|------|
| Code Interpreter 完整链路 | LLM → code_interpreter_skill → Sidecar → 文件生成 |
| 错误重试 | 代码执行失败后 LLM 自动修复重试 |
| 确认流程 | 用户确认/拒绝 Code Interpreter 操作 |
| 精简后 Skill | read/convert/analyze 通过现有 Skill 正常工作 |
