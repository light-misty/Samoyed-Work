...
由于 Samoyed Work 的 Sidecar 是 Python 进程，无法直接执行 JavaScript（docx-js、pptxgenjs），因此采用以下整合策略：
...
Samoyed Work 的 Sidecar 是 Python 进程，通过 stdin/stdout JSON 协议与 Rust 后端通信。引入 JavaScript 会需要：
